...
/*   Updated: 2023/05/19 12:40:16 by tlegrand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_create_vector(t_game *game)
{
	...
	while (i < game->mlx.win_height)
	{
		...
		{
			r_h = 2 * tan((60 * PI / 180) * 0.5) / game->mlx.win_width;
			r_v = 2 * tan((60 * PI / 180) * game->mlx.win_height / (game->mlx.win_width * 2)) / game->mlx.win_height;
			game->rays[i][j].x = ((j - game->mlx.win_width * 0.5) * r_h);
			game->rays[i][j].y = -1.0;
			game->rays[i][j].z = ((game->mlx.win_height * 0.5 - i) * r_v);
			j++;
		}
		i++;
	}
